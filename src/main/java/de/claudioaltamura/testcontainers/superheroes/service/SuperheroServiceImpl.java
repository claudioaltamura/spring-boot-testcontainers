...
public class SuperheroServiceImpl implements SuperheroService {
  ...
  public SuperheroServiceImpl(
      SuperheroRepository superheroRepository, SuperheroMapper superheroMapper) {
  ...
  public Superhero getById(@NotNull Long id) {
    return superheroRepository
        ...
        .orElseThrow(
            () ->
                new SuperheroNotFoundException(String.format("Superhero (id=%d) not found.", id)));
  }
  ...
  public List<Superhero> findByName(@NotNull String name) {
    return superheroRepository.findByName(name).stream()
        .map(superheroMapper::toDto)
        .collect(Collectors.toList());
  }
